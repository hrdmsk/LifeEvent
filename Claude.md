...
Better Auth のテーブルは `@better-auth/cli generate` で生成し、`migrations/0003_betterauth_worldid.sql` に取り込んでいる。CLI用の設定は `auth.config.ts`（`better-sqlite3` を使う。実行時には無関係）。プラグイン追加等でスキーマが変わったら再生成して新しいマイグレーションに反映する。
...
```
src/                     React フロントエンド
  App.tsx                useSession で LoginScreen / Home を出し分け
  authClient.ts          better-auth/react クライアント
  api.ts                 /api/me/* のクライアント
  components/            LoginScreen(World IDボタン) / Home / EventForm / Timeline
worker/src/
...
migrations/
  0001_init / 0002_auth / 0003_betterauth_worldid.sql
auth.config.ts           Better Auth CLI（スキーマ生成）専用
```

### データモデル（D1: `lifeevent`）

- Better Auth 管理: **user** / **session** / **account** / **verification**（id は TEXT）
- アプリ: **life_events**（`user_id` は TEXT で `user.id` 参照）/ **records**（日付の追記専用台帳・INSERTのみ）
...
1. 本番D1に `npm run migrate:remote`。
2. `wrangler secret put BETTER_AUTH_URL` / `BETTER_AUTH_SECRET` / `WORLDID_APP_ID` / `WORLDID_CLIENT_SECRET`。
3. World ID Developer Portal のリダイレクトURIに `https://<domain>/api/auth/oauth2/callback/worldid` を登録。
4. `npm run deploy`。
