...
src/                     React フロントエンド（react-router）
  ...
  pages/                 AnniversariesPage(/) / AnniversaryPage(/a/:uuid・経過表示) / LoginPage(/login) / HomePage(/me)
  components/            NavBar / PublicAnniversaries / Home / EventForm / Timeline / SavedTokens
worker/src/
  ...
  users.ts               EventStore（life_events のクエリ、uuid採番）
  ledger.ts              Ledger インターフェース + D1実装（records）
  tokens.ts              TokenStore（saved_tokens：UUIDのコレクション）
  types.ts               Env / Variables
migrations/
  auth/0001_betterauth.sql              AUTH_DB 用
  app/0001_events.sql / 0002_public_rate_limit.sql / 0003_tokens.sql   APP_DB 用
...
- **AUTH_DB**（lifeevent_auth, `env.AUTH_DB`）: Better Auth 管理の **user** / **session** / **account** / **verification**（id は TEXT）
- **APP_DB**（lifeevent_app, `env.APP_DB`）:
  - **life_events**（`user_id` は TEXT、`uuid` = 記念日ごとの一意ID＝将来のSBT Token IDの代替）
  - **records**（日付の追記専用台帳・INSERTのみ）
  - **saved_tokens**（アカウントが保持する Token ID(UUID) のコレクション。SBTの「所有」の先取り）
...
| GET | `/api/public/anniversaries/:uuid` | 記念日1件（専用ページ `/a/:uuid` 用・公開） |
| GET/POST | `/api/me/tokens` | 保持するToken ID(UUID)の一覧 / 保存 |
| DELETE | `/api/me/tokens/:uuid` | 保存を解除 |
